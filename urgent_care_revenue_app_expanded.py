...
if uploaded_file:
    ...
    summary['Performance Category'] = summary['Normalized Residual'].apply(label)
    ...
    st.subheader("🏥 Payer Mix Contribution")
    payer_mix = df.groupby(['ISO Week of Visit Service Date', 'Primary Financial Class'])['Payments + Expected'].sum().reset_index()
    payer_pivot = payer_mix.pivot(index='ISO Week of Visit Service Date', columns='Primary Financial Class', values='Payments + Expected').fillna(0)
    st.line_chart(payer_pivot)

    st.subheader("📋 CPT Code Revenue Impact")
    cpt_rev = df.groupby(['ISO Week of Visit Service Date', 'Chart CPT Code'])['Payments + Expected'].sum().reset_index()
    cpt_pivot = cpt_rev.pivot(index='ISO Week of Visit Service Date', columns='Chart CPT Code', values='Payments + Expected').fillna(0)
    st.line_chart(cpt_pivot)

    st.subheader("🔬 Complexity & Lab Trends")
    fig3, ax3 = plt.subplots()
    ax3.plot(summary['ISO Week of Visit Service Date'], summary['Avg. Chart E/M Weight'], label='Complexity')
    ax3.plot(summary['ISO Week of Visit Service Date'], summary['Labs per Visit'], label='Labs per Visit')
    ax3.set_title("Visit Complexity & Labs per Visit")
    ax3.set_xticks(range(len(summary)))
    ax3.set_xticklabels(summary['ISO Week of Visit Service Date'], rotation=45)
    ax3.legend()
    st.pyplot(fig3)
